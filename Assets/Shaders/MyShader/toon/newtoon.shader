Shader "Shader Forge/newtoon" {
    Properties {
      ...
      [HDR] _EmitColor("EmitColor", Color) = (1.0, 0.3, 0.2)
            ...
            _EmitMap ("EmitMap", 2D) = "white" {}
      [HDR] _EmitMapColor ("EmitMapColor", Color) = (1,1,1)
            [Toggle] _EMITMAPENABLE("EMITMAPENABLE",Float) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            #pragma shader_feature_local _EMITMAPENABLE_ON
            ...
            float4 frag(VertexOutput i) : COLOR {
               ...
               half3 worldLightDir = mainLight.direction;
                ...
                diffuse *= mainTex.xyz;
                half3 emitColor = _EmitColor * _EmitInt;
                #if defined _EMITMAPENABLE_ON
                    emitColor += tex2D(_EmitMap, i.uv).rgb * _EmitMapColor;
                #endif

                // 环境光反射代码 1 是粗糙的 0.1 是 occlusion
                // half3 reflectDirWS = reflect(-viewDir, worldNormal);//normalWS为像素法线
                // half3 environmentReflections = GlossyEnvironmentReflection(reflectDirWS, 1, 0.1);
                // col.rgb = (diffuse.rgb + specular + rimColor + environmentReflections) * finalLight + emitColor;
                
                col.rgb = (diffuse.rgb + specular + rimColor) * finalLight + emitColor;
                // 雾效
