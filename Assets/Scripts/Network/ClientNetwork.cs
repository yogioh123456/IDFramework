...
public class ClientNetwork : IFixedUpdate,IApplicationQuit
{
    ...
    public ClientNetwork() {
        ...
        client.CreateMessageHandlersDictionary(GetType().Assembly);
        
        //连接服务器
        //Connect("127.0.0.1", 7778);
        //注册
        //Game.Get<ClientNetwork>().Send(NetMsg.MessageId.Register, "username", "password");
        //登录
        //Game.Get<ClientNetwork>().Send(NetMsg.MessageId.Login, "username", "password");
    }
    ...
    /// 发送给其他玩家
    /// </summary>
    public void Cmd(string msgName, byte[] datas) {
        Game.ClientNet.Send(NetMsg.MessageId.Broadcast, msgName, datas);
    }
