...
public static class Util {
    ...
    {
        var bin = MemoryPackSerializer.Deserialize<T>(v);
        return bin;
    }
    
    public static Vector2 GetPointByRadian(float x, float y, float angle) {
        var newPoint = new Vector2();
        var radian = (Mathf.PI / 180f) * angle;
        newPoint.x = x * Mathf.Cos(radian) - y * Mathf.Sin(radian);
        newPoint.y = x * Mathf.Sin(radian) + y * Mathf.Cos(radian);
        return newPoint;
    }
    
    public static bool NewCheckYuanXing(Vector3 checkPos, Vector3 targetPos, float checkDistance) {
        var distance = Mathf.Pow(targetPos.x - checkPos.x, 2) + Mathf.Pow(targetPos.z - checkPos.z, 2);
    ...
    public static bool NewCheckShanXing(Vector3 startPos, Vector3 targetPos, float checkRotation, float checkDistance, float localRotationEulerY) {
        float offsetX = targetPos.x - startPos.x;
        float offsetY = targetPos.z - startPos.z;
        var distance = Mathf.Pow(offsetX, 2) + Mathf.Pow(offsetY, 2);
        if (distance < checkDistance * checkDistance) {
            var rotation = Mathf.Atan2(offsetX, offsetY) * 180 / Mathf.PI;
            if (Mathf.Abs(rotation - localRotationEulerY) < checkRotation || Mathf.Abs(rotation - localRotationEulerY) > (360 - checkRotation))
                return true;
        }
    ...
    /** 新 长就是长长 宽就长宽*/
    public static bool NewCheckJuXing(Vector3 startPos, float rotation, float width, float height, Vector3 targetPos, float targetrange, bool isCenter = false) {
        var tx = targetPos.x - startPos.x;
        var ty = targetPos.z - startPos.z;
        var newPoint = GetPointByRadian(tx, ty, rotation);
        ...
        var cy = newPoint.y;
        //找出x方向与cx最接近的
        var minx = Mathf.Min(Mathf.Abs(x1 - cx), Mathf.Abs(x2 - cx));
        //找出y方向与cy最接近的
        var miny = Mathf.Min(Mathf.Abs(y1 - cy), Mathf.Abs(y2 - cy));
        if (minx * minx + miny * miny < targetrange * targetrange)
            return true;

        //条件2
        var x0 = (x1 + x2) / 2;
        var y0 = (y1 + y2) / 2;
        if ((Mathf.Abs(x0 - cx) < Mathf.Abs(x2 - x1) / 2 + targetrange) && Mathf.Abs(cy - y0) < Mathf.Abs(y2 - y1) / 2)
            return true;

        if ((Mathf.Abs(y0 - cy) < Mathf.Abs(y2 - y1) / 2 + targetrange) && Mathf.Abs(cx - x0) < Mathf.Abs(x2 - x1) / 2)
            return true;
